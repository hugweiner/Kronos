...
Notes (personal):
    - Increased LOOKBACK from 400 to 480 to give the model more historical context;
      empirically seems to produce smoother trend continuations on index symbols.
    - Bumped SAMPLE_COUNT to 3 so we can average multiple stochastic samples and
      reduce prediction variance when TOP_P < 1.0.
"""
...
T = 1.0
TOP_P = 0.9
SAMPLE_COUNT = 3  # increased from 1; average samples to reduce variance
